...
class TestSkProphet(TestCase):
    ...
    def test_get_params_deep(self):
        ...
        self.assertTrue(
            params['sk_extra_regressors'] is not sk_extra_regressors)
        self.assertTrue(
            params['sk_extra_regressors'][0] is not sk_extra_regressors[0])
...
class TestStepsSelectorEstimator(TestCase):
    ...
    def test_fit_sorted(self):
        ...
        X_call, y_call = args
        self.assertEqual(
            [[i] for i in range(15, 30) for _ in range(2)],
            X_call.values.tolist())
        self.assertEqual(
            list(range(14, -1, -1)) + list(range(15, 30, 1)),
            y_call.values.tolist())
